...
void query_5(CATALOG *c, int format_flag, char **args, int args_size, GList **resList) {
    *resList = NULL;
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        char *flight_origin_upper = toupper_str(get_flight_info(flight, 4));
        ...
        if (strcmp(flight_origin_upper, airport_name_upper) == 0) {
            ...
            if (date_comparator_wt(get_flight_info(flight, 6), begin_date) >= 0 && date_comparator_wt(get_flight_info(flight, 7), end_date) <= 0) {
                char* flightStr = NULL;

                if (format_flag) { // Format the output
                    char *str = "id: %s\nschedule_departure_date: %s\ndestination: %s\nairline: %s\nplane_model: %s\n\n";
                    int len = snprintf(NULL, 0, str, get_flight_id(flight), get_flight_info(flight, 6), flight_destination_upper, get_flight_info(flight, 1), get_flight_info(flight, 2));
                    flightStr = g_malloc(len + 1);
                    snprintf(flightStr, len + 1, str, get_flight_id(flight), get_flight_info(flight, 6), flight_destination_upper, get_flight_info(flight, 1), get_flight_info(flight, 2));
                } else {
                    char *str = "%s;%s;%s;%s;%s\n";
                    int len = snprintf(NULL, 0, str, get_flight_id(flight), get_flight_info(flight, 6), flight_destination_upper, get_flight_info(flight, 1), get_flight_info(flight, 2));
                    flightStr = g_malloc(len + 1);
