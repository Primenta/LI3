...
void query_9(CATALOG *c, int format_flag, char **args, int args_size, GList **usersList) {
    *usersList = NULL;
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (strncmp(get_user_info(user, 1), prefix, strlen(prefix)) == 0) {
            // check if user is active
            if (isActive(get_user_info(user, 11)) == 0) continue;

            //printf("User found: %s for prefix %s\n", get_user_info(user, 1), prefix);
            char* userStr = NULL;

            if (format_flag) {
                char *str = "id: %s\nname: %s\n\n";
                int len = snprintf(NULL, 0, str, get_user_id(user), get_user_info(user, 1));
                userStr = g_malloc(len + 1);
                snprintf(userStr, len + 1, str, get_user_id(user), get_user_info(user, 1));
            } else {
                char *str = "%s;%s\n";
                int len = snprintf(NULL, 0, str, get_user_id(user), get_user_info(user, 1));
                userStr = g_malloc(len + 1);
