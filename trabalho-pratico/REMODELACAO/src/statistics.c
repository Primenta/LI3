...
int count_number_flights(GHashTable *passengers, const char *user_id) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        for (int i = 0; i < get_flight_seats_total_passengers(flight_seats); i++) {
            if (strcmp(get_flight_seats_passengers(flight_seats)[i], user_id) == 0) {
                count++;
            }
        }
    }
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        for (int i = 0; i < get_flight_seats_total_passengers(flight_seats); i++) {
            if (strcmp(get_flight_seats_passengers(flight_seats)[i], user_id) == 0) {
                ...
                flights[flights_count] = g_strdup(set_flight_seats_id(flight_seats));
                flights_count++;
            }
        }
    }
...
int count_number_reservations(GHashTable *reservations, const char *user_id) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (strcmp(get_reservation_info(reservation, 1), user_id) == 0) {
            count++;
        }
    }
    return count;
...
char** get_user_reservations(GHashTable *reservations, const char *user_id) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (strcmp(get_reservation_info(reservation, 1), user_id) == 0) {
            ...
            reservations_array[reservations_count] = g_strdup(get_reservation_id(reservation));
            reservations_count++;
        }
    }
...
double calculate_total_spent(GHashTable *reservations, const char *user_id) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (strcmp(get_reservation_info(reservation, 1), user_id) == 0) {
            ...
            int number_of_nights = calculate_nights(get_reservation_info(reservation, 7), get_reservation_info(reservation, 8));
            // calculate the total spent
            double price_per_night = atof(get_reservation_info(reservation, 9));
            double city_tax = atof(get_reservation_info(reservation, 5));
