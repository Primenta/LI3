    ...
    *reservationsList = NULL;
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        if (strcmp(get_reservation_info(reservation, 2), args[0]) == 0) {
            double total_price = calculate_total_price(get_reservation_info(reservation, 9), calculate_nights(get_reservation_info(reservation, 7), get_reservation_info(reservation, 8)), atoi(get_reservation_info(reservation, 5)));
            char* reservationStr = NULL;

            if (format_flag) { // Format the output
                char *str = "id: %s\nbegin_date: %s\nend_date: %s\nuser_id: %s\nrating: %s\ntotal_price: %.3f\n\n";
                int len = snprintf(NULL, 0, str, get_reservation_id(reservation), get_reservation_info(reservation, 7), get_reservation_info(reservation, 8), get_reservation_info(reservation, 1), get_reservation_info(reservation, 11), total_price);
                reservationStr = g_malloc(len + 1);
                snprintf(reservationStr, len + 1, str, get_reservation_id(reservation), get_reservation_info(reservation, 7), get_reservation_info(reservation, 8), get_reservation_info(reservation, 1), get_reservation_info(reservation, 11), total_price);
            } else {
