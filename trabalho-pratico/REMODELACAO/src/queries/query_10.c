...
void query_10(CATALOG *c, int format_flag, char **args, int args_size, GList **resList) {
    *resList = NULL;
    ...
    if (args_size == 0) {
        ...
        g_hash_table_iter_init(&iter2, getFlights(c));

        while (g_hash_table_iter_next(&iter2, &key2, &value2)) {
            FLIGHT *flight = (FLIGHT *)value2;

            // get the year from the schedule_departure_date
            char *schedule_departure_date_copy = g_strdup(get_flight_info(flight, 6));
        ...
        g_hash_table_iter_init(&iter4, getPassengers(c));
        ...
        while (g_hash_table_iter_next(&iter4, &key4, &value4)) {
            ...
            g_hash_table_iter_init(&iter5, getFlights(c));

            while (g_hash_table_iter_next(&iter5, &key5, &value5)) {
                FLIGHT *flight = (FLIGHT *)value5;
                if (strcmp(get_flight_id(flight), set_flight_seats_id(flight_seats)) == 0) {
                    // get the year from the schedule_departure_date
                    char *schedule_departure_date_copy = g_strdup(get_flight_info(flight, 6));
                    char *year_str = strtok(schedule_departure_date_copy, "/");
                    // add the passengers to the list
                    add_to_ymetrics(&years_metrics, year_str, get_flight_seats_total_passengers(flight_seats), 3);
